...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 29, 2023 at 02:25 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateTransactionTotalPriceByID` (IN `trans_id` INT, OUT `totalPrice` INT)   BEGIN
    UPDATE transaksi
    JOIN car ON transaksi.idRentCar_tbnote = car.idRentCar
    JOIN jadwalhotel ON transaksi.idjadwalhotel_tbnote = jadwalhotel.idjadwalhotel
    JOIN kamar ON jadwalhotel.idRoom_tbjh = kamar.idRoom
    JOIN jadwaldestination ON transaksi.idjadwaldes_tbnote = jadwaldestination.idjadwaldes
    JOIN destination ON jadwaldestination.idDestination_tbjd = destination.idDestination
    SET transaksi.totalPrice = (car.priceMobil * transaksi.JumlahSewaMobil) + 
        ((CASE 
            WHEN transaksi.PaketyangDipilih = '3 Hari' THEN kamar.priceRoom * 3
            WHEN transaksi.PaketyangDipilih = '7 Hari' THEN kamar.priceRoom* 7
            ELSE kamar.priceRoom*1
        END) * transaksi.JumlahKamar) + (destination.price * transaksi.JumlahOrang)
    WHERE transaksi.idTransaksi = trans_id;
    SELECT transaksi.totalPrice INTO totalPrice FROM transaksi where idTransaksi= trans_id limit 1;
END$$
...
(507, 457, 217, 447, '2023-01-07', '2023-01-14', '7 Hari', 1, 1, 1, 'YES', 425000),
